...
"Secured" debt is whatever each tranche's `TrancheConfig.is_secured`
resolves to (type-based default, or an explicit override) -- see
`assumptions/schema.py`.
"""
...
class CreditMetrics:
    ...
    secured_net_leverage: np.ndarray
    interest_coverage: np.ndarray
    fccr: np.ndarray
    cumulative_debt_paydown: np.ndarray


def secured_debt_balance(
    debt_schedule: DebtScheduleResult, tranches: list[TrancheConfig]
) -> np.ndarray:
    secured_names = [t.name for t in tranches if t.is_secured]
    return sum(debt_schedule.ending_balance[name] for name in secured_names)
...
def compute_credit_metrics(
    ...
    secured_debt = secured_debt_balance(debt_schedule, tranches)
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        return CreditMetrics(
            ...
            secured_net_leverage=(secured_debt - cash) / ebitda,
